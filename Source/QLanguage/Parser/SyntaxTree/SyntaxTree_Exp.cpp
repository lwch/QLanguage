...
namespace QLanguage
{
    SyntaxTree_Exp::SyntaxTree_Exp(SyntaxTree_Base& OP1, SyntaxTree_Base* pOP2, SyntaxTree_Base* pOP3)
    ...
    SyntaxTree_Exp::SyntaxTree_Exp(Type type, SyntaxTree_Base& OP1, SyntaxTree_Base* pOP2)
        : parent(sizeof(SyntaxTree_Exp))
        , OP1(OP1)
        , pOP2(pOP2)
        , pOP3(NULL)
    ...
    SyntaxTree_Exp::SyntaxTree_Exp(Type type, SyntaxTree_Base& OP)
        : parent(sizeof(SyntaxTree_Exp))
        , OP1(OP)
        , pOP2(NULL)
        , pOP3(NULL)
    ...
    bool SyntaxTree_Exp::make(Parser* pParser)
    {
        ...
        case Value:
            {
                if (dynamic_cast<SyntaxTree_Value&>(OP1).isConstValue())
                {
                    VM::Variant v = dynamic_cast<SyntaxTree_Value&>(OP1).toVariant();
                ...
                {
                    pair<short, ushort> p = pParser->indexOfRegister(dynamic_cast<SyntaxTree_Value&>(OP1).name());
                    if (p.first == -1)
                    {
                        string str = string::format("undefined variable %s", dynamic_cast<SyntaxTree_Value&>(OP1).value.c_str());
        ...
        case TrueFalse:
            {
                ...
                if (OP1.isConstValue())
                {
                    if (dynamic_cast<SyntaxTree_Exp&>(OP1).toVariant(pParser).toBool())
                    {
                        if (!pOP2->make(pParser)) return false;
                        if (pOP2->isConstValue()) // 可提前计算出结果
                        {
                            ucBlock = dynamic_cast<SyntaxTree_Exp*>(pOP2)->ucBlock;
                            usIndex = dynamic_cast<SyntaxTree_Exp*>(pOP2)->usIndex;
                            bConstant = true;
                        }
                    }
                    else
                    {
                        ...
                        {
                            ucBlock = dynamic_cast<SyntaxTree_Exp*>(pOP3)->ucBlock;
                            usIndex = dynamic_cast<SyntaxTree_Exp*>(pOP3)->usIndex;
                            bConstant = true;
                        }
                    }
                }
                else
                {
                    ...
                    i.op = VM::OpCode::Jmp;
                    i.ot = MAKE_OT(0, 0, 0);
                    i.Jmp.ob  = dynamic_cast<SyntaxTree_Exp&>(OP1).ucBlock;
                    i.Jmp.os  = dynamic_cast<SyntaxTree_Exp&>(OP1).usIndex;
                    i.Jmp.ext = true;
                    ...
                    {
                        if (!make_constant(pParser, *dynamic_cast<SyntaxTree_Exp*>(pOP2), p)) return false;
                    }
                    else
                    {
                        if (!make_register(pParser, *dynamic_cast<SyntaxTree_Exp*>(pOP2), p)) return false;
                    }
                    ...
                    {
                        if (!make_constant(pParser, *dynamic_cast<SyntaxTree_Exp*>(pOP3), p)) return false;
                    }
                    else
                    {
                        if (!make_register(pParser, *dynamic_cast<SyntaxTree_Exp*>(pOP3), p)) return false;
                    }
                    if (!insert_flag_register(pParser, p)) return false; // mov本身并不设置flag寄存器，应此需要手动设置
                    pParser->instructions[ri].Jmp.addr = pParser->instructions.size() - ri - 1;
                    ucBlock = (uchar)p.first;
        ...
        case Negative:
            {
                ...
                if (OP1.isConstValue())
                {
                    VM::Variant v = eval(dynamic_cast<SyntaxTree_Exp&>(OP1).toVariant(pParser), _type);
                    const pair<int, ushort> p = pParser->indexOfConstant(v);
                    if (p.first == -1)
                    {
                        const pair<short, ushort> pa = pParser->pushConstant(v);
                ...
                else return make_op1(pParser, pair<uchar, ushort>(dynamic_cast<SyntaxTree_Exp&>(OP1).ucBlock, dynamic_cast<SyntaxTree_Exp&>(OP1).usIndex), _type);
            }
            break;
        case GreaterEqual:
        case LessEqual:
        ...
        case Mod:
            {
                ...
                {
                    VM::Variant v = eval(dynamic_cast<SyntaxTree_Exp&>(OP1).toVariant(pParser),
                                         dynamic_cast<SyntaxTree_Exp*>(pOP2)->toVariant(pParser),
                                         _type);
                    const pair<int, ushort> p = pParser->indexOfConstant(v);
                    if (p.first == -1)
                    {
                ...
                else
                {
                    if (bConstValue1)
                        return make_op2(pParser,
                                        pair<uchar, ushort>(dynamic_cast<SyntaxTree_Exp&>(OP1).ucBlock, dynamic_cast<SyntaxTree_Exp&>(OP1).usIndex),
                                        *dynamic_cast<SyntaxTree_Exp*>(pOP2),
                                        _type);
                    else if (bConstValue2)
                        return make_op2(pParser,
                                        dynamic_cast<SyntaxTree_Exp&>(OP1),
                                        pair<uchar, ushort>(dynamic_cast<SyntaxTree_Exp*>(pOP2)->ucBlock, dynamic_cast<SyntaxTree_Exp*>(pOP2)->usIndex),
                                        _type);
                    else return make_op2(pParser, dynamic_cast<SyntaxTree_Exp&>(OP1), *dynamic_cast<SyntaxTree_Exp*>(pOP2), _type);
                }
            }
            break;
        default:
    ...
    template <typename T1, typename T2, typename T3, typename T4>
    inline void construct(T1* p, const T2& v1, T3& v2, T4* v3)
    {
        new (p) T1(v1, v2, v3);
    }
